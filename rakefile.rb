...
task :ci => [:update_all_dependencies, :default, :history, :publish, "template:build", "nuget:build"]
...
task :compile => [:restore_if_missing, :clean, :version] do
  ...
  bottles("create-pak getting-started build/basic-getting-started.zip -target #{COMPILE_TARGET}")
end
...
task :unit_test => :compile do
  ...
  runner.executeTests ['FubuMVC.Tests', 'FubuMVC.WebForms.Testing', 'FubuMVC.Spark.Tests', 'FubuMVC.Diagnostics.Tests']
end
